...
class CoreGraphicsViewModel {
    ...
    init() {
        self.spikes = spikes
        Timer.publish(every: 0.2,
                      on: .main,
                      in: .common)
        .autoconnect()
        .sink { date in
            // update spikes
            (self.spikes < self.maximumSpikes) ? (self.spikes += 1) : (self.spikes = self.minimumSpikes)

            (self.wavesAmplitude < self.maximumAmplitude) ? (self.wavesAmplitude += 3) : (self.wavesAmplitude = self.minimumAmplitude)
        }
...
struct CoreGraphics: View {
    ...
    private var starShape: some View {
        StarburstShape(spikes: viewModel.spikes, innerRadiusRatio: 0.75 - (0.05 * CGFloat(viewModel.spikes)))
    ...
    private var waveShape: some View {
        WaveShape(amplitude: viewModel.wavesAmplitude,
                  frequency: 5)
            .fill(LinearGradient(
                gradient: Gradient(colors: [.blue, .cyan, .purple, .yellow.opacity(0.5)]),
                startPoint: .top,
                endPoint: .bottom)
            )
            .frame(height: 200)
    ...
    private var frameShape: some View {
        Image("turtlerock_feature")
            ...
            .overlay {
                MyFrame()
                    .stroke(.linearGradient(Gradient(colors: [.blue, .green, .white, .red]), startPoint: .bottomTrailing, endPoint: .topLeading),
